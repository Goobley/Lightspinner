...
from formal_solver import piecewise_linear_1d, IPsi, piecewise_linear_1d_single_up
...
class UV:
    ...
    """
    Uji: Union[np.ndarray, float]
    Vij: Union[np.ndarray, float]
    Vji: Union[np.ndarray, float]
...
class ComputationalTransition:
    ...
    zero_rates(self)
        Zeros radiative rates, Rij and Rji.
    """
    ...
    def __init__(self, trans: Union[AtomicLine, AtomicContinuum], compAtom: 'ComputationalAtom', atmos: Atmosphere, spect: SpectrumConfiguration):
        ...
        if isinstance(trans, AtomicLine):
            ...
            self.sahaBoltzFactor = trans.iLevel.g / (2 * trans.jLevel.g) \
                                    * (Const.HPlanck**2 / (2 * np.pi * Const.MElectron * Const.KBoltzmann * atmos.temperature))**1.5 \
                                    * np.exp((trans.jLevel.E_SI - trans.iLevel.E_SI) / (Const.KBoltzmann * atmos.temperature))
    ...
    def uv_k(self, la, mu, toFrom, k) -> UV:
        """Compute the Uji, Vij and Vji coefficients from [RH92] and [U01]
        for this transition for an wavelength, angle and direction, for a single
        depth point.
        ...
        if self.isLine:
            phi = self.phi[lt, mu, int(toFrom), k]
            Vij = hc_4pi * self.Bij * phi
            Vji = self.gij[k] * Vij
            Uji = self.Aji / self.Bji * Vji
        else:
...
class ComputationalAtom:
    ...
    zero_rates(self)
        Zero the radiative rates (Rij, Rji) of all transitions in this atom.
    """
    ...
    def setup_wavelength(self, laIdx: int):
        ...
        for kr, t in enumerate(self.trans):
            ...
            if t.isLine:
                ...
                # self.gij[kr, :] = self.atmos.ne * t.sahaBoltzFactor \
                #     * np.exp(-hc_k / self.spect.wavelength[laIdx] / self.atmos.temperature)
                self.wla[kr, :] = t.wlambda(t.lt(laIdx)) / self.spect.wavelength[laIdx] / Const.HPlanck

    def setup_wavelength_k(self, laIdx: int, k: int):
        """Configure the scratch matrices for the coming wavelength (of index
        laIdx in the global array) and precalculate the terms that are
        constant over angle (gij & wla).
        """
        ...
        for kr, t in enumerate(self.trans):
            ...
            if t.isLine:
                ...
                # off by a factor of \lambda, but aren't due to the use Doppler
                # units (inside wlambda).
                self.gij[kr, k] = t.Bji / t.Bij
                self.wla[kr, k] = t.wlambda(t.lt(laIdx)) * t.wphi[k] / Const.HC
            else:
                self.gij[kr, k] = self.nStar[t.i, k] / self.nStar[t.j, k] \
                    * np.exp(-hc_k / self.spect.wavelength[laIdx] / self.atmos.temperature[k])
                # self.gij[kr, :] = self.atmos.ne * t.sahaBoltzFactor \
                #     * np.exp(-hc_k / self.spect.wavelength[laIdx] / self.atmos.temperature)
                self.wla[kr, k] = t.wlambda(t.lt(laIdx)) / self.spect.wavelength[laIdx] / Const.HPlanck
...
class Context:
    ...
    def formal_sol_gamma_matrices(self, fixCol=False):
        """Perform the formal solution over all specified wavelengths and angles
        and fill in the Gamma matrix.
        ...
        dJ = np.abs(1.0 - self.J / self.Jnew)
        dJMax = dJ.max()

        return dJMax

    def formal_sol_gauss_seidel(self, fixCol=False, sorOmega=1.0):
        '''N.B. will only work with the linear method for now
        '''
        Nspace = self.atmos.Nspace
        Nrays = self.atmos.Nrays
        Nspect = self.spect.wavelength.shape[0]
        ...
        # NOTE(cmo): Sweep back up doing stat equil at each depth to implicitly
        # use the upper triangular form of the ALO
        # Handle lower boundary first
        Iup = np.zeros((Nspect, Nrays, Nspace))
        chiUp = np.zeros((Nspect, Nrays, Nspace))
        etaUp = np.zeros((Nspect, Nrays, Nspace))
        SUp = np.zeros((Nspect, Nrays, Nspace))
        z = self.atmos.height
        dPops = 0.0
        toFrom = True
        k = Nspace - 1
            ...
            for la, wav in enumerate(self.spect.wavelength):
                for atom in self.activeAtoms:
                    ...
                    for kr, t in enumerate(atom.trans):
                        if not t.active[la]:
                            continue
                        uv = t.uv_k(la, mu, toFrom, k-1)
                        chi = atom.n[t.i, k-1] * uv.Vij - atom.n[t.j, k-1] * uv.Vji
                        chiUp[la, mu, k-1] += chi
                        eta = atom.n[t.j, k-1] * uv.Uji
                        etaUp[la, mu, k-1] += eta

                        uv = t.uv_k(la, mu, toFrom, k)
                        chi = atom.n[t.i, k] * uv.Vij - atom.n[t.j, k] * uv.Vji
                        chiUp[la, mu, k] += chi
                        eta = atom.n[t.j, k] * uv.Uji
                        etaUp[la, mu, k] += eta

                chiUp[la, mu, k-1] += background.chi[la, k]
                chiUp[la, mu, k] += background.chi[la, k]
                etaUp[la, mu, k-1] += background.eta[la, k]
                etaUp[la, mu, k] += background.eta[la, k]

                zmu = 1.0 / self.atmos.muz[mu]
                dtau_uw = zmu * (chiUp[la, mu, -1] + chiUp[la, mu, -2]) * 0.5 * np.abs(z[-1] - z[-2])
                Bnu = planck(self.atmos.temperature[-2:], wav)
                Ibottom = Bnu[1] - (Bnu[0] - Bnu[1]) / dtau_uw
                self.Jnew[la, -1] += 0.5 * self.atmos.wmu[mu] * Ibottom
                Iup[la, mu, k] = Ibottom
                for atom in self.activeAtoms:
                    for kr, t in enumerate(atom.trans):
                        if not t.active[la]:
                            continue
                        wmu = 0.5 * self.atmos.wmu[mu]
                        wlamu = atom.wla[kr, k] * wmu * 4 * np.pi

                        uv = t.uv_k(la, mu, toFrom, k)
                        Ieff = Ibottom

                        integrand = uv.Uji + uv.Vji * Ieff
                        ...
                        # NOTE(cmo): Radiative rates component for this angle and frequency
                        # as per (28) of [U01] or (2.8) or [RH92]
                        t.Rij[k] += Ibottom * uv.Vij * wlamu
                        t.Rji[k] += (uv.Uji + Ibottom * uv.Vji) * wlamu
        self.finish_gamma_k(k)
        dPopsk = self.stat_equil_k(k, sorOmega=sorOmega)
        dPops = max(dPops, dPopsk)
        ...
        for mu in range(Nrays):
            for la, wav in enumerate(self.spect.wavelength):
                ...
                SUp[la, mu, k] = (etaUp[la, mu, k] +
                     background.sca[la, k] * self.Jnew[la, k]) / chiUp[la, mu, k]

        dk = -1
        for k in range(Nspace-2, -1, dk):
            for mu in range(Nrays):
                for la, wav in enumerate(self.spect.wavelength):
                    chiUp[la, mu, k] = 0.0
                    etaUp[la, mu, k] = 0.0
                    ...
                    chiTot += background.chi[la, k]
                    etaTot += background.eta[la, k]
                    S = (etaTot +
                        background.sca[la, k] * self.J[la, k]) / chiTot

                    chiUp[la, mu, k] = chiTot
                    etaUp[la, mu, k] = etaTot
                    SUp[la, mu, k] = S

                    Ik, Psik = piecewise_linear_1d_single_up(z, self.atmos.muz[mu], chiUp[la, mu], SUp[la, mu], Iup[la, mu, k-dk], k)
                    Iup[la, mu, k] = Ik

                    self.Jnew[la, k] += 0.5 * self.atmos.wmu[mu] * Ik
                    ...
                    for atom in self.activeAtoms:
                        for kr, t in enumerate(atom.trans):
                            ...
                            uv = t.uv_k(la, mu, toFrom, k)
                            Ieff = Ik - Psik * (atom.n[t.j, k] * uv.Uji)
                            chi = atom.n[t.i, k] * uv.Vij - atom.n[t.j, k] * uv.Vji
                            integrand = (1.0 - chi * Psik) * uv.Uji + uv.Vji * Ieff
                            atom.Gamma[t.i, t.j, k] += integrand * wlamu
                            ...
                            # NOTE(cmo): Radiative rates component for this angle and frequency
                            # as per (28) of [U01] or (2.8) or [RH92]
                            t.Rij[k] += Ik * uv.Vij * wlamu
                            t.Rji[k] += (uv.Uji + Ik * uv.Vji) * wlamu
            ...
            for mu in range(Nrays):
                for la, wav in enumerate(self.spect.wavelength):
                    ...
                    etaTot += background.eta[la, k]
                    S = (etaTot +
                        background.sca[la, k] * self.Jnew[la, k]) / chiTot

                    chiUp[la, mu, k] = chiTot
                    etaUp[la, mu, k] = etaTot
                    SUp[la, mu, k] = S
                    Ik, _ = piecewise_linear_1d_single_up(z, self.atmos.muz[mu], chiUp[la, mu], SUp[la, mu], Iup[la, mu, k-dk], k)
                    self.Jnew[la, k] += 0.5 * self.atmos.wmu[mu] * (Ik - Iup[la, mu, k])
                    Iup[la, mu, k] = Ik
        self.I[...] = Iup[:, :, 0]
    ...
    def stat_equil_k(self, k, sorOmega=1.0):
        """Update the populations of all active species towards statistical
        equilibrium, using the current version of the Gamma matrix, for a single
        depth point k.
        ...
        for atom in self.activeAtoms:
            ...
            change = np.abs(1.0 - nOld / nNew)
            maxRelChange = max(maxRelChange, change.max())
            atom.n[:, k] = nOld + sorOmega * (nNew - nOld)
    ...
    def nr_post_update(self, sorOmega=1.0):
        ...
        for k in range(Nspace):
            ...
            start = 0
            for atom in self.activeAtoms:
                atom.n[:, k] += sorOmega * update[start:start+atom.Nlevel]
                start += atom.Nlevel
            self.atmos.ne[k] += sorOmega * update[-1]
            pops = np.zeros(Neqn)
            start = 0
            for atom in self.activeAtoms:
